...
void mtxMultiplyNewParallelTbb(crsmtx a, crsmtx b, crsmtx &c, double& time)
{
	clock_t start = clock();


	tbb::parallel_for(static_cast<size_t>(0), static_cast<size_t>(c.n), [&](size_t i) {
		int *x = new int[a.m];
		memset(x, -1, sizeof(int)*a.m);
...
int main()
{
	...
	mtxMultiplyNewParallelTbb(a, bt, cp, time2);
